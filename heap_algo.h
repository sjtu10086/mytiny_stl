#ifndef ZZF_HEAP_ALGO_H_
#define ZZF_HEAP_ALGO_H_
...
{
  zzf_stl::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0), *(last - 1));
}
...
{
  zzf_stl::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0),
                       *(last - 1), comp);
}
...
{
  zzf_stl::push_heap_d(first, last, distance_type(first), comp);
}
...
void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value)
{
  ...
  zzf_stl::push_heap_aux(first, holeIndex, topIndex, value);
}
                 ...
                 Compared comp)
{
  ...
  // 再执行一次上溯(percolate up)过程
  zzf_stl::push_heap_aux(first, holeIndex, topIndex, value, comp);
}
                  ...
                  T value, Distance*, Compared comp)
{
  *result = *first;  // 先将尾指设置成首值，即尾指为欲求结果
  zzf_stl::adjust_heap(first, static_cast<Distance>(0), last - first, value, comp);
}

template <class RandomIter, class Compared>
void pop_heap(RandomIter first, RandomIter last, Compared comp)
{
  zzf_stl::pop_heap_aux(first, last - 1, last - 1, *(last - 1),
...
void sort_heap(RandomIter first, RandomIter last, Compared comp)
{
  while (last - first > 1)
  {
    zzf_stl::pop_heap(first, last--, comp);
  }
}
...
void make_heap_aux(RandomIter first, RandomIter last, Distance*)
{
  ...
  {
    // 重排以 holeIndex 为首的子树
    zzf_stl::adjust_heap(first, holeIndex, len, *(first + holeIndex));
    if (holeIndex == 0)
      return;
    holeIndex--;
  }
...
void make_heap_aux(RandomIter first, RandomIter last, Distance*, Compared comp)
{
  ...
  {
    // 重排以 holeIndex 为首的子树
    zzf_stl::adjust_heap(first, holeIndex, len, *(first + holeIndex), comp);
    if (holeIndex == 0)
      return;
    holeIndex--;
  }
...
{
  zzf_stl::make_heap_aux(first, last, distance_type(first), comp);
}
